...
describe('generic Interactable schema', () => {
  ...
  it('rejects invalid types (invalid type instead of InteractionMode)', () => {
    expectInvalidFixture(definition, { interactions: { foo: 'qux' } }, [{ type: 'expectedType', name: 'interactions.foo' }]);
  });

  it('rejects invalid types (invalid object, not InteractionMode)', () => {
    expectInvalidFixture(definition, { interactions: { foo: { qux: 1 } } }, [{ type: 'keyNotInSchema', name: 'interactions.foo.qux' }]);
  });
